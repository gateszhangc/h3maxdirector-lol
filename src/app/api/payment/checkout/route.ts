  ...
  try {
    ...
    if (!paymentProviderName) {
      paymentProviderName =
        ...
        (configs.waffo_enabled === 'true' ? 'waffo' : '');
    }
    if (!paymentProviderName) {
...
async function getPaymentProductId(
  ...
  if (provider !== 'creem' && provider !== 'waffo') {
    // currently only creem and waffo support payment product id mapping
    return;
  }

  try {
    const configs = await getAllConfigs();
    const productIdConfig =
      provider === 'creem'
        ? configs.creem_product_ids
        : configs.waffo_product_ids;

    if (productIdConfig) {
      const productIds = JSON.parse(productIdConfig);
      return (
