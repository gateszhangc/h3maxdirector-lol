...
export function SignModal({ callbackUrl = '/' }: { callbackUrl?: string }) {
  ...
  const { isShowSignModal, setIsShowSignModal, setSignCallbackUrl } =
    useAppContext();
  ...
  if (isDesktop) {
    return (
      <Dialog open={isShowSignModal} onOpenChange={handleOpenChange}>
        <DialogContent className="dark h3-brand text-foreground sm:max-w-[425px]">
          <DialogHeader>
  ...
  return (
    <Drawer open={isShowSignModal} onOpenChange={handleOpenChange}>
      <DrawerContent className="dark h3-brand text-foreground">
